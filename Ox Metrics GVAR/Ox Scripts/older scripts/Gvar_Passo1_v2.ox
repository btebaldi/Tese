		...
		for(iContCols = 0; iContCols<=iQtdLags; ++iContCols)
		{
			...
			for(iContVar = 0; iContVar < sizeof(asTipo); ++iContVar)
			{
				...
				if(iContCols == 0)
				{
					...
					sParamName = sprint("star_", asTipo[iContVar], "@", sVarSufix, iRegDependente, "_", asTipo[iContRows]);
				}
				else
				{
					// Determina o nome do parametro COM lag
					sParamName = sprint("star_", asTipo[iContVar], "_", iContCols, "@", sVarSufix, iRegDependente, "_", asTipo[iContRows]);
				}
...
ProcessoIIS(const iValue, const sName, const iRegDependente, const sVarSufix)
{
	...
	nContTotal = 0;
	for(nCountAno = 2004; nCountAno <= 2016; ++nCountAno)
	{
...
main()
{
	...
	decl iQtdVarDependente, iQtdRegioes, iQtdLags, txDbase, txCoIntMatPath, sVarSufix, txMatPathRawMatrix, txMatPathA_Matrix, txMatPathW_Matrix;
	...
	for (iCont = 1; iCont <= iQtdRegioes; ++iCont)
	{
		...
		println("Iniciando construcao das variaveis star para a regiao ", iCont);
		// MELHORAR ESSE PROCESSO NO FUTURO!!!
		decl mData, beta;
		mData = model.GetAll();
		...
		//println(model.GetVar(sprint("R", iCont, "_Admitidos")));
		...
		model.Append(diff(mData), {sprint("D_R", iCont, "_Desligados"), sprint("D_R", iCont, "_Admitidos"), "D_star_Desligados", "D_star_Admitidos"});

		// CONTRUCAO DA MATRIZ DE LONGO PRAZO
		println("Iniciando construcao da variavel beta*Z (Cointegracao) para a regiao ", iCont);
	   ...
	   // INICIAMOS A MODELAGEM DO VAR PARA ESTIMACAO
		...
		model.Select("Y", {sprint(sVarSufix, iCont,"_Admitidos"), 0, 0});
		model.Select("Y", {sprint(sVarSufix, iCont,"_Desligados"), 0, 0});
		...
		for (iCont2 = 1; iCont2 <= 5; ++iCont2)
		{
			...
			if(iCont2 == iCont)
			{
				...
				model.Select("X", {sprint(sVarSufix, iCont2,"_Admitidos"), 1, iQtdLags});
				model.Select("X", {sprint(sVarSufix, iCont2,"_Desligados"), 1, iQtdLags});
				}
			}
			else
			{
		...
		decl mPhi, mLambda, mLambda_0, mU, iContParam, iTotalParam, asParamNames, vParamValues, nContLags, A_0, A_L, mIIS, mAlpha;
		...
		//Impressao dos parametros em Tela
		//print( "%r", asParamNames,"%f", vParamValues );
		
		// Salva a matriz de IIS Para o modelo 
		mIIS = ProcessoIIS(vParamValues, asParamNames, iCont, sVarSufix);
		savemat(sprint(txMatPathRawMatrix, sVarSufix, iCont, "_IIS.mat"), mIIS);

		mAlpha = ProcessoPILongRun(vParamValues, asParamNames, iCont, sVarSufix);
		savemat(sprint(txMatPathRawMatrix, sVarSufix, iCont, "_Alpha.mat"), mAlpha);

		// Completa os valores da matrix Lambda
		mLambda = ProcessoLambda(mLambda, vParamValues, asParamNames, iQtdLags, iCont, sVarSufix);
		savemat(sprint(txMatPathRawMatrix, sVarSufix, iCont, "_Lambda.mat"), mLambda);

		mPhi = ProcessoPhi(mPhi, vParamValues, asParamNames, iQtdLags, iCont, sVarSufix);
		savemat(sprint(txMatPathRawMatrix, sVarSufix, iCont, "_Phi.mat"), mPhi);

		mU = ProcessoU(mU, vParamValues, asParamNames, iQtdLags, iCont, sVarSufix);
		//print(mU);
		savemat(sprint(txMatPathRawMatrix, sVarSufix, iCont, "_U.mat"), mU);
