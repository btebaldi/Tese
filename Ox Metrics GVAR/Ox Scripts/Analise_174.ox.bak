...
main()
{
	...
	model.Select("Y", {"R289_Admitidos", 0, 0});
	model.Select("Y", {"R289_Desligados", 0, 0});
	model.Select("Y", {"star_dem2", 0, 0});
	model.Select("Y", {"star_adm2", 0, 0});
...
//	model.Select("U", {"Constant", 0, 0});
	...
	model.Restrict({"[beta]","[alpha]","* * 0 0","* * 0 0"});
	model.Restrict({"[beta]","* * * *","0 0 * *","[alpha]","* * 0 0","0 0 * *"});
