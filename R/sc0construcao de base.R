...
Muni_Micro.info <- readxl::read_excel("Database/Cadastro Municipios.xlsx", sheet = "TabelaCompleta", range = "A1:E5613")
...
Muni_Micro.info2 <- Muni_Micro.info %>%
  dplyr::left_join(ArranjoPopulacional.Info, by=c("ID_Municipio"="Codmun")) %>% 
  dplyr::mutate(ID_MicroIBGE = ID_Micro)


selectionVector <- !is.na(Muni_Micro.info2$ID_Micro_AP)

Muni_Micro.info2$ID_Micro[selectionVector] <- Muni_Micro.info2$ID_Micro_AP[selectionVector]


setdiff(aaa$ID_Micro, bbb$ID_Micro)
